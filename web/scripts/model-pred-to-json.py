...
def main():
  ...
  # Prompt the user to get the path to the model root
  model_root = input('Enter the path to the models folder: ')

  # Get the highest number of the experiment
  exp_number = get_highest_exp_number(os.path.join(model_root + '/runs/detect/'))

  print('The highest number of the experiment is: {}'.format(exp_number))

  # Get the path to the experiment
  exp_path = os.path.join(model_root, 'runs/detect/exp{}'.format(exp_number))
