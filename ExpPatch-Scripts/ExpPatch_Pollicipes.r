...
policip<-subset(StatsSite,Species=='Pollicipes_polymerus')

#Create single graph for Pollicipes polymerus (or whatever species you want)
pdf("ExpPatch-Output/Pollicipes-Dynamics.pdf",height=4,width=6)
par(mfrow=c(1,1))
    maxy<-1.2*max(policip$Density[,'Mean']+policip$Density[,'SE'],na.rm=TRUE)
    plot(policip$Date,(policip$Density[,'Mean']+policip$Density[,'SE']), xlab="",ylab='Density (#/m^2)', type='n', main='', xaxt='n', ylim=c(0,maxy))
    xat<-seq(min(policip$Date), max(policip$Date), "months")
    axis.Date(1,at=xat,las=2,format="%b")
